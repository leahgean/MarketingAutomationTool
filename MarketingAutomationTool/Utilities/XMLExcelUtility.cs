...
{
    public class XMLExcelUtility
    {
        public bool WriteDataTableToExcel(Guid UserId, DataTable sourceTable, string OutputFileDirectory, string saveAsLocation)
        {

            try
            {
                CreateExcelFile(sourceTable, OutputFileDirectory, saveAsLocation);
                return true;
            }
            catch (Exception ex)
            {
                Logger.Logger.WriteLog(UserId.ToString(), "XMLExcelUtility-WriteDataTableToExcel", string.Empty, "Error", ex.Message);
                return false;
            }
        }


        private void CreateExcelFile(DataTable data, string OutPutFileDirectory, string fileFullname)
        {
        ...
        private void GenerateWorkbookStylesPartContent(WorkbookStylesPart workbookStylesPart1)
        {
            ...
            Color headercolor = new Color() { Rgb="FFFFFF" };
            ...
            ForegroundColor foregroundColor1 = new ForegroundColor() { Rgb = "3F3F3F" };
            ...
            CellFormat cellFormat5 = new CellFormat() { NumberFormatId = (UInt32Value)0U, FontId = (UInt32Value)2U, FillId = (UInt32Value)2U, BorderId = (UInt32Value)0U, FormatId = (UInt32Value)0U, ApplyFill = true };
        ...
        private SheetData GenerateSheetdataForDetails(DataTable data)
        {
            ...
            sheetData1.Append(CreateHeaderRowForExcel(data.Columns));

            foreach (DataRow dr in data.Rows)
            {
                Row partsRows = GenerateRowForChildPartDetail(data.Columns, dr);
                sheetData1.Append(partsRows);
            }
            return sheetData1;
        }
        private Row CreateHeaderRowForExcel(DataColumnCollection dc)
        {
            Row workRow = new Row();
            string excludecolumns = ConstantValues.LeadsExportColumnsToExclude;
            string[] excludecolumnsArray = excludecolumns.Split('|');
            bool blnIncludeColumn = true;
            foreach (DataColumn colname in dc)
            {
                foreach (string excludecolumn in excludecolumnsArray)
                {
                    if (colname.ColumnName.Trim().ToLower() == excludecolumn.Trim().ToLower())
                    {
                        blnIncludeColumn = false;
                        break;
                    }
                }
                if (blnIncludeColumn) workRow.Append(CreateCell(colname.ColumnName, (UInt32Value)3U));
                blnIncludeColumn = true;
            }
               
            return workRow;
        }
        private Row GenerateRowForChildPartDetail(DataColumnCollection dc, DataRow dr)
        {
            Row tRow = new Row();
            ...
            foreach (DataColumn colname in dc)
            {
                ...
                if (blnIncludeColumn) tRow.Append(CreateCell(dr[colname].ToString()));
                blnIncludeColumn = true;
            }
            return tRow;
        }
